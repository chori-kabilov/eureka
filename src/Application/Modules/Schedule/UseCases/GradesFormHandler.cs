...
public class GetGradesFormHandler
{
    private readonly IDataContext _db;

    public GetGradesFormHandler(IDataContext db) => _db = db;
    ...
    public async Task<Result<GradesFormDto>> HandleAsync(Guid lessonId, CancellationToken ct = default)
    {
        ...
        {
            gradingSystem = await _db.GradingSystems.FirstOrDefaultAsync(g => g.IsDefault, ct);
        }
